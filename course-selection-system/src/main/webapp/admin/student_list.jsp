...
<body>
    ...
    <div class="container">
        <div class="page-header">
            ...
            <div class="toolbar">
                <form class="search-box" method="get" action="student"><input type="hidden" name="action" value="list"><input type="text" name="keyword" placeholder="搜索学号或姓名" value="${param.keyword}"><button type="submit">搜索</button></form>
                <a href="student?action=template" class="btn btn-outline">下载模板</a>
                <button type="button" class="btn btn-outline" onclick="document.getElementById('importFile').click()">导入Excel</button>
                <a href="student?action=export" class="btn btn-outline">导出Excel</a>
                <a href="student?action=add" class="btn btn-primary">添加学生</a>
            </div>
        </div>
        <form id="importForm" action="student" method="post" enctype="multipart/form-data" style="display:none">
            <input type="hidden" name="action" value="import">
            <input type="file" id="importFile" name="file" accept=".xlsx,.xls" onchange="this.form.submit()">
        </form>
        <c:if test="${param.msg == 'reset_success'}"><div class="alert">密码重置成功，新密码为：123456</div></c:if>
        ...
        <table>
            ...
            <c:forEach items="${students}" var="stu">
                <tr>
                    <td class="student-no">${stu.studentNo}</td><td>${stu.name}</td><td>${stu.gender}</td><td>${stu.className}</td><td>${stu.phone}</td><td>${stu.email}</td>
                    <td>
                        <a href="student?action=edit&id=${stu.id}" class="btn btn-warning btn-sm">编辑</a>
                        <form action="student" method="post" style="display:inline"><input type="hidden" name="action" value="resetPwd"><input type="hidden" name="id" value="${stu.id}"><button type="submit" class="btn btn-outline btn-sm" onclick="return confirm('确定重置密码为123456？')">重置</button></form>
                        <form action="student" method="post" style="display:inline" onsubmit="return confirm('确定删除？')"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="${stu.id}"><button type="submit" class="btn btn-danger btn-sm">删除</button></form>
                    </td>
                </tr>
            </c:forEach>
            <c:if test="${empty students}"><tr><td colspan="7"><div class="empty">暂无学生数据</div></td></tr></c:if>
            </tbody>
        </table>
    </div>
</body>
