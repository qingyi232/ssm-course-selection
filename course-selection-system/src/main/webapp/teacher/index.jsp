...
<body>
    ...
    <div class="container">
        <div class="welcome">
            <h2>你好，${sessionScope.user.name} 老师</h2>
            <p>欢迎使用在线课程选课系统</p>
        </div>
        <div class="stats-grid">
            <div class="stat-card"><div class="number">${courseCount != null ? courseCount : 0}</div><div class="label">开设课程</div></div>
            <div class="stat-card"><div class="number">${totalStudents != null ? totalStudents : 0}</div><div class="label">选课学生</div></div>
            <div class="stat-card"><div class="number">${avgRate != null ? avgRate : '0.0'}%</div><div class="label">平均选课率</div></div>
            <div class="stat-card"><div class="number red">${onlineCount != null ? onlineCount : 1}</div><div class="label">在线人数</div></div>
        </div>
        <div class="section-title">快捷入口</div>
        <div class="quick-grid">
            <a href="course?action=list" class="quick-card"><div class="title">课程管理</div><div class="desc">管理我的课程</div><div class="arrow">→</div></a>
            <a href="course?action=add" class="quick-card"><div class="title">添加课程</div><div class="desc">创建新课程</div><div class="arrow">→</div></a>
            <a href="score?action=list" class="quick-card"><div class="title">成绩管理</div><div class="desc">录入学生成绩</div><div class="arrow">→</div></a>
            <a href="profile" class="quick-card"><div class="title">个人信息</div><div class="desc">查看修改信息</div><div class="arrow">→</div></a>
        </div>
        <div class="grid-2">
            <div class="card">
                <div class="card-title">个人信息</div>
                <div class="info-row"><span class="label">工号</span><span class="value">${sessionScope.user.teacherNo}</span></div>
                ...
                <div class="info-row"><span class="label">职称</span><span class="value">${sessionScope.user.title}</span></div>
                <div class="info-row"><span class="label">学院</span><span class="value">${sessionScope.user.collegeName}</span></div>
                <div class="info-row"><span class="label">电话</span><span class="value">${sessionScope.user.phone}</span></div>
            </div>
            <div class="card">
